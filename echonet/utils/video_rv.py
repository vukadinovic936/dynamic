...
def run(num_epochs=45,
    ...
    mean, std = echonet.utils.get_mean_and_std(echonet.datasets.Echo_RV_Video(split="train"))
    kwargs = {"mean": mean,
              "std": std,
              "length": frames,
              "period": period,
    ...
    train_dataset = echonet.datasets.Echo_RV_Video(split="train", **kwargs)
    ...
    val_dataloader = torch.utils.data.DataLoader(
        echonet.datasets.Echo_RV_Video(split="val", **kwargs), batch_size=batch_size, num_workers=num_workers, shuffle=True, pin_memory=(device.type == "cuda"))
    ...
    with open(os.path.join(output, "log.csv"), "a") as f:
        ...
        for epoch in range(epoch_resume, num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                loss, yhat, y = echonet.utils.video_rv.run_epoch(model, dataloaders[phase], phase == "train", optim, device)
                f.write("{},{},{},{},{},{},{},{},{}\n".format(epoch,
                                                              phase,
                                                              loss,
        ...
        if run_test:
            for split in ["val", "test"]:
                ...
                dataloader = torch.utils.data.DataLoader(
                    echonet.datasets.Echo_RV_Video(split=split, **kwargs),
                    batch_size=batch_size, num_workers=num_workers, shuffle=True, pin_memory=(device.type == "cuda"))
                loss, yhat, y = echonet.utils.video_rv.run_epoch(model, dataloader, False, None, device)
                ...
                ds = echonet.datasets.Echo_RV_Video(split=split, **kwargs, clips="all")
                dataloader = torch.utils.data.DataLoader(
                    ds, batch_size=1, num_workers=num_workers, shuffle=False, pin_memory=(device.type == "cuda"))
                loss, yhat, y = echonet.utils.video_rv.run_epoch(model, dataloader, False, None, device, save_all=True, block_size=batch_size)
    ...
    with torch.set_grad_enabled(train):
        with tqdm.tqdm(total=len(dataloader)) as pbar:
            for (X, outcome) in dataloader:
                ...
                loss = torch.nn.functional.mse_loss(outputs.view(-1).float(), outcome.float())
